 ...
 * @author jflute
 */
public class SigninAction extends MigueldoraBaseAction {
    ...
    public HtmlResponse signin(SigninForm form) {
        ...
        return migueldoraLoginAssist.loginRedirect(form.account, form.password, op -> op.rememberMe(form.rememberMe), () -> {
            return redirect(RootAction.class);
        });
    }

    private void moreValidate(SigninForm form, MigueldoraMessages messages) {
        if (isNotEmpty(form.account) && isNotEmpty(form.password)) {
            if (!migueldoraLoginAssist.checkUserLoginable(form.account, form.password)) {
