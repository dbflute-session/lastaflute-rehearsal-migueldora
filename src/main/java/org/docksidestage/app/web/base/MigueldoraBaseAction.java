...
import org.docksidestage.app.web.base.login.MigueldoraLoginAssist;
import org.docksidestage.app.web.base.paging.PagingNavi;
import org.docksidestage.app.web.base.paging.SearchPagingBean;
import org.docksidestage.mylasta.action.MigueldoraHtmlPath;
import org.docksidestage.mylasta.action.MigueldoraMessages;
 ...
 * @author jflute
 */
public abstract class MigueldoraBaseAction extends TypicalAction // has several interfaces for direct use
        implements LaValidatable<MigueldoraMessages>, MigueldoraHtmlPath {
    ...
    @Resource
    private MigueldoraLoginAssist migueldoraLoginAssist;
    @Resource
    ...
    public void hookFinally(ActionRuntime runtime) { // application may override
        if (runtime.isForwardToHtml()) {
            runtime.registerData("headerBean", getUserBean().map(userBean -> {
                return new MigueldoraHeaderBean(userBean);
            }).orElse(MigueldoraHeaderBean.empty()));
        }
    ...
    //                                                                           =========
    @Override
    protected OptionalThing<MigueldoraUserBean> getUserBean() { // to return as concrete class
        return migueldoraLoginAssist.getSessionUserBean(); // #app_customize return empty if login is unused
    }
    ...
    protected OptionalThing<LoginManager> myLoginManager() {
        return OptionalThing.of(migueldoraLoginAssist);
    }
    ...
    @Override
    public ActionValidator<MigueldoraMessages> createValidator() { // for co-variant
        return super.createValidator();
    }

    @Override
    public MigueldoraMessages createMessages() { // application may call
        return new MigueldoraMessages(); // overriding to change return type to concrete-class
    }
    ...
    protected PagingNavi createPagingNavi(PagingResultBean<? extends Entity> page, Object form) { // application may override
        return new PagingNavi(page, op -> {
            op.rangeSize(migueldoraConfig.getPagingPageRangeSizeAsInteger());
    ...
    protected int getPagingPageSize() { // application may call
        return migueldoraConfig.getPagingPageSizeAsInteger();
    }
