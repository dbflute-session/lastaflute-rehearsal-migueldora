 ...
 * @author FreeGen
 */
public interface MigueldoraEnv {
    ...
    /** The key of the configuration. e.g. /tmp/lastaflute/migueldora */
    ...
    /** The key of the configuration. e.g. migueldora-support@annie.example.com */
     ...
     * The value is, e.g. /tmp/lastaflute/migueldora <br>
     ...
     * The value is, e.g. migueldora-support@annie.example.com <br>
     * comment: Mail Address for Migueldora Support
     * @return The value of found property. (NotNull: if not found, exception but basically no way)
     */
     ...
     * @author FreeGen
     */
    public static class SimpleImpl extends ObjectiveConfig implements MigueldoraEnv {

        /** The serial version UID for object serialization. (Default) */
        private static final long serialVersionUID = 1L;

        public String getLastaDiSmartDeployMode() {
            return get(MigueldoraEnv.lasta_di_SMART_DEPLOY_MODE);
        }

        public String getDevelopmentHere() {
            return get(MigueldoraEnv.DEVELOPMENT_HERE);
        }

        public boolean isDevelopmentHere() {
            return is(MigueldoraEnv.DEVELOPMENT_HERE);
        }

        public String getEnvironmentTitle() {
            return get(MigueldoraEnv.ENVIRONMENT_TITLE);
        }

        public String getFrameworkDebug() {
            return get(MigueldoraEnv.FRAMEWORK_DEBUG);
        }

        public boolean isFrameworkDebug() {
            return is(MigueldoraEnv.FRAMEWORK_DEBUG);
        }

        public String getTimeAdjustTimeMillis() {
            return get(MigueldoraEnv.TIME_ADJUST_TIME_MILLIS);
        }

        public Long getTimeAdjustTimeMillisAsLong() {
            return getAsLong(MigueldoraEnv.TIME_ADJUST_TIME_MILLIS);
        }

        public String getLogLevel() {
            return get(MigueldoraEnv.LOG_LEVEL);
        }

        public String getLogConsoleLevel() {
            return get(MigueldoraEnv.LOG_CONSOLE_LEVEL);
        }

        public String getLogFileBasedir() {
            return get(MigueldoraEnv.LOG_FILE_BASEDIR);
        }

        public String getMailSendMock() {
            return get(MigueldoraEnv.MAIL_SEND_MOCK);
        }

        public boolean isMailSendMock() {
            return is(MigueldoraEnv.MAIL_SEND_MOCK);
        }

        public String getMailSmtpServerMainHostAndPort() {
            return get(MigueldoraEnv.MAIL_SMTP_SERVER_MAIN_HOST_AND_PORT);
        }

        public String getMailSubjectTestPrefix() {
            return get(MigueldoraEnv.MAIL_SUBJECT_TEST_PREFIX);
        }

        public String getMailReturnPath() {
            return get(MigueldoraEnv.MAIL_RETURN_PATH);
        }

        public String getMailAddressSupport() {
            return get(MigueldoraEnv.MAIL_ADDRESS_SUPPORT);
        }

        public String getJdbcDriver() {
            return get(MigueldoraEnv.JDBC_DRIVER);
        }

        public String getJdbcUrl() {
            return get(MigueldoraEnv.JDBC_URL);
        }

        public String getJdbcUser() {
            return get(MigueldoraEnv.JDBC_USER);
        }

        public String getJdbcPassword() {
            return get(MigueldoraEnv.JDBC_PASSWORD);
        }

        public String getJdbcConnectionPoolingSize() {
            return get(MigueldoraEnv.JDBC_CONNECTION_POOLING_SIZE);
        }

        public Integer getJdbcConnectionPoolingSizeAsInteger() {
            return getAsInteger(MigueldoraEnv.JDBC_CONNECTION_POOLING_SIZE);
        }

        public String getServerDomain() {
            return get(MigueldoraEnv.SERVER_DOMAIN);
        }
    }
}
