...
import org.docksidestage.mylasta.direction.MigueldoraEnv;
import org.lastaflute.core.direction.exception.ConfigPropertyNotFoundException;

/**
 * @author FreeGen
 */
public interface MigueldoraConfig extends MigueldoraEnv {

    /** The key of the configuration. e.g. migueldora */
    ...
    String COOKIE_REMEMBER_ME_MIGUELDORA_KEY = "cookie.remember.me.migueldora.key";
     ...
     * The value is, e.g. migueldora <br>
    ...
    /**
     * Get the value for the key 'cookie.remember.me.migueldora.key'. <br>
     * The value is, e.g. HRB <br>
     * comment: The cookie key of remember-me for Migueldora
     * @return The value of found property. (NotNull: if not found, exception but basically no way)
     */
    String getCookieRememberMeMigueldoraKey();
     ...
     * @author FreeGen
     */
    public static class SimpleImpl extends MigueldoraEnv.SimpleImpl implements MigueldoraConfig {

        /** The serial version UID for object serialization. (Default) */
        private static final long serialVersionUID = 1L;

        public String getDomainName() {
            return get(MigueldoraConfig.DOMAIN_NAME);
        }

        public String getDomainTitle() {
            return get(MigueldoraConfig.DOMAIN_TITLE);
        }

        public String getCookieDefaultPath() {
            return get(MigueldoraConfig.COOKIE_DEFAULT_PATH);
        }

        public String getCookieDefaultExpire() {
            return get(MigueldoraConfig.COOKIE_DEFAULT_EXPIRE);
        }

        public Integer getCookieDefaultExpireAsInteger() {
            return getAsInteger(MigueldoraConfig.COOKIE_DEFAULT_EXPIRE);
        }

        public String getCookieEternalExpire() {
            return get(MigueldoraConfig.COOKIE_ETERNAL_EXPIRE);
        }

        public Integer getCookieEternalExpireAsInteger() {
            return getAsInteger(MigueldoraConfig.COOKIE_ETERNAL_EXPIRE);
        }

        public String getCookieRememberMeMigueldoraKey() {
            return get(MigueldoraConfig.COOKIE_REMEMBER_ME_MIGUELDORA_KEY);
        }

        public String getPagingPageSize() {
            return get(MigueldoraConfig.PAGING_PAGE_SIZE);
        }

        public Integer getPagingPageSizeAsInteger() {
            return getAsInteger(MigueldoraConfig.PAGING_PAGE_SIZE);
        }

        public String getPagingPageRangeSize() {
            return get(MigueldoraConfig.PAGING_PAGE_RANGE_SIZE);
        }

        public Integer getPagingPageRangeSizeAsInteger() {
            return getAsInteger(MigueldoraConfig.PAGING_PAGE_RANGE_SIZE);
        }

        public String getPagingPageRangeFillLimit() {
            return get(MigueldoraConfig.PAGING_PAGE_RANGE_FILL_LIMIT);
        }

        public boolean isPagingPageRangeFillLimit() {
            return is(MigueldoraConfig.PAGING_PAGE_RANGE_FILL_LIMIT);
        }
    }
}
